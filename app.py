...
class User(Base):
    ...
    diet_logs = relationship('User_Diet_Log', back_populates='user')
    meal_plans = relationship('Meal_Plan', back_populates='user')
    feedback = relationship('Feedback', back_populates='user')
    weight_history = relationship('User_Weight_History', back_populates='user')
...
# --- NEW: Admin-only User Management Routes (MODIFIED) ---
...
def admin_update_user(user_id):
    ...
    # --- MODIFICATION: Allow admin to reset password ---
    if 'Password' in data and data['Password']: # Check for non-empty password
        new_hashed_password = bcrypt.generate_password_hash(data['Password']).decode('utf-8')
...
def admin_delete_user(user_id):
    # --- MODIFICATION: Prevent admin from deleting themselves ---
    ...
    # --- MODIFICATION: Add try/except for safer deletion ---
    try:
        db.session.delete(user)
        db.session.commit()
        ...
        db.session.rollback()
        # This will send a specific error to the frontend
        return jsonify({"error": f"Failed to delete user. Database error: {str(e)}"}), 500
    # --- END MODIFICATION ---


# --- Recipe CRUD (No changes, still user-specific) ---
...
def get_recipes():
    ...
    recipes = Recipe.query.filter_by(Creator_User_ID=user_id).all()
...
def get_recipe(recipe_id):
    ...
    current_user_id = get_jwt_identity()
    if recipe.Creator_User_ID != current_user_id:
         return jsonify({"error": "Unauthorized"}), 403
...
def update_recipe(recipe_id):
    ...
    for key, value in data.items():
        if hasattr(recipe, key) and key != "Creator_User_ID":
...
# --- Ingredient CRUD (No changes, all users can use) ---
...
# --- Meal_Plan CRUD (No changes, user-specific) ---
...
def get_mealplans():
    user_id = get_jwt_identity()
    plans = Meal_Plan.query.filter_by(User_ID=user_id).all()
...
def get_mealplan(plan_id):
    ...
    ).filter_by(MealPlan_ID=plan_id, User_ID=user_id).first()
...
def update_ingredient_in_recipe(ri_id):
    ...
    user_id = get_jwt_identity()
    if ri.recipe.Creator_User_ID != user_id:
        return jsonify({"error": "Unauthorized"}), 403
...
def remove_recipe_from_mealplan(mpr_id):
    ...
    user_id = get_jwt_identity()
    if mpr.meal_plan.User_ID != user_id:
        return jsonify({"error": "Unauthorized"}), 403
