-- =========================================================
-- NUTRITIONDB_FINAL.SQL  (100% aligned with Flask backend)
-- =========================================================
...
CREATE TABLE IF NOT EXISTS Recipe_Log (
  ...
  Created_By INT,
  Created_At TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  CONSTRAINT fk_recipelog_recipe FOREIGN KEY (Recipe_ID)
    REFERENCES Recipe(Recipe_ID)
    ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB;
...
CREATE TABLE IF NOT EXISTS User_Weight_History (
  ...
  CONSTRAINT fk_weighthist_user FOREIGN KEY (User_ID)
    REFERENCES User(User_ID)
    ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB;

-- If you haven't created the linking table yet, create it with these fields:
CREATE TABLE IF NOT EXISTS MealPlan_Recipes (
    MPR_ID INT AUTO_INCREMENT PRIMARY KEY,
    MealPlan_ID INT NOT NULL,
    Recipe_ID INT NOT NULL,
    Meal_Type VARCHAR(32) DEFAULT NULL,        -- Breakfast/Lunch/Dinner/Snack
    Day_Of_Week VARCHAR(16) DEFAULT NULL,      -- Monday..Sunday (for weekly planner)
    Sort_Order INT DEFAULT 0,                  -- ordering index for display
...
BEGIN
  ...
  JOIN Nutrition n ON n.Ingredient_ID = i.Ingredient_ID
...
BEGIN
  SELECT 
    ...
    mpr.Meal_Type,
    r.Recipe_Name,
    GetRecipeCalories(r.Recipe_ID) AS Recipe_Calories
  FROM Meal_Plan mp
  JOIN MealPlan_Recipe mpr ON mp.MealPlan_ID = mpr.MealPlan_ID
  JOIN Recipe r ON r.Recipe_ID = mpr.Recipe_ID
  WHERE mp.User_ID = userId
  ORDER BY mpr.Day_of_Plan, mpr.Meal_Type;
END;
//
...
BEGIN
  ...
  SELECT Weight_kg INTO oldWeight FROM User WHERE User_ID = p_userId;
  UPDATE User
  SET Weight_kg = p_newWeight
